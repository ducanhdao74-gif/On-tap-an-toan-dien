...
def load_data():
    ...
    try:
        ...
        for sheet in sheet_names:
            df = pd.read_excel(xls, sheet=NameError0 := sheet) # Đọc sheet
            # Chuẩn hóa tên cột cơ bản nếu cần
            # Giả định cột: Câu hỏi, Đáp án A, B, C, D, Đáp án đúng, Chủ đề/Phần
            all_questions.append(df)
            
        # Gộp dữ liệu các sheet nếu có nhiều phần
...
if error_message:
    ...
    if mode == "Học theo chủ đề":
        ...
        # Thêm logic hiển thị câu hỏi ở đây cho ông
    else:
